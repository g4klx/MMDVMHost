...
{
    display.fillRect(0, 10, display.width(), 10, BLACK);
    display.setCursor(0,10);
...
void COLED::writeDMR(unsigned int slotNo, const std::string& src, bool group, const std::string& dst, const char* type)
{
    ...
    if (m_mode != MODE_DMR) {
        ...
        if (slotNo == 1U)
        {
          display.fillRect(0, 30, display.width(), 10, BLACK);
          display.setCursor(0,30);
          display.print("2 Listening");
          display.fillRect(0, 40, display.width(), 10, BLACK);
        }
        else
        {
          display.fillRect(0, 10, display.width(), 10, BLACK);
          ...
          display.fillRect(0, 20, display.width(), 10, BLACK);
        }
    }
...
void COLED::OLED_statusbar()
{
    ...
    if (m_mode == MODE_DMR)
      display.drawBitmap(0, 0, logo_dmr_bmp, 24, 10, WHITE);
    else
      display.drawBitmap(0, 0, logo_glcd_bmp, 16, 10, WHITE);
}
