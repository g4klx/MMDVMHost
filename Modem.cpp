...
bool CModem::writeI2CCommand(unsigned char address, const unsigned char* data, unsigned int length)
{
	assert(m_port != NULL);
	assert(data != NULL);
	assert(length > 0U);
	...
	m_port->write(buffer, length + 5U);

	return true;
}

bool CModem::writeI2CData(unsigned char address, const unsigned char* data, unsigned int length)
{
	assert(m_port != NULL);
	assert(data != NULL);
	assert(length > 0U);
